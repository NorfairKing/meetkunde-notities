...
\begin{st}
  ...
  \begin{proof}
    ...
    Omdat $t_{b}$ een isometrie is\stref{st:translatie-isometrie} en de samenstelling van isometrie\"en intern\stref{st:samenstelling-isometrieen-intern}, moeten we enkel bewijzen dat $G$ een isometrie is.
...
\begin{st}
  ...
  \end{itemize}
  \begin{proof}
    Er bestaat een unieke affiene transformatie $F$ die aan de eisen voldoet.\stref{st:rakende-ruimtes-transformatie}
...
\begin{gev}
  ...
  \begin{proof}
    ...
    Er bestaat dan een unieke $F$ die aan de voorwaarden voldoet.\stref{st:bestaan-isometrie}
...
\begin{de}
  Zij $S=p+V$ een hypervlak, en $\{u\}$ een basis van $V^{\bot}$ met \'e\'en element, dan noemen we $u$ een \term{normaal} op $S$.
\end{de}
