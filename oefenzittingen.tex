...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[scale=1,extended line/.style={shorten >=-#1,shorten <=-#1},extended line/.default=1cm] 
    ...
    \draw [extended line=4cm, red] (a) -- +($(p)-(ap)$) node[above,text=red] {$l_{a}$};
    \draw [extended line=6.5cm, green] (b) -- +($(p)-(bp)$) node[above,text=green] {$l_{b}$};
    \draw [extended line=7.5cm, blue] (c) -- +($(p)-(cp)$) node[above,text=blue] {$l_{c}$};

    \coordinate [label=right:$q$] (q) at ($ (z)!-2!(p) $);  
  \end{tikzpicture}
...
\item Zij $H$ de homothetie met centrum $z$ die $a'$ afbeeldt op $a$, dan beeldt $H$ ook $b$ op $b'$ af en $c$ op $c'$.
  $H$ beeldt ook $pa'$ op $l_{a}$ af, $pb'$ op $l_{b}$ en $pc'$ op $l_{c}$.\stref{st:colineariteit-affien-invariant}
  $l_{a}$,$l_{b}$ en $l_{c}$ zijn dus concurrent.stref{st:concurrentie-affien-invariant}
\item Omdat de afbeelding van $p$ onder $H$ $q$ is, zijn $p$, $q$ en $z$ colineair.
\item Er geldt bovendien $(z,p,q)=-2$.
  Merk eerst op dat $z$, $p$ en $q$ colineair zijn, dus het is zinvol om over $(z,p,q)$ te spreken.-
\end{enumerate}
